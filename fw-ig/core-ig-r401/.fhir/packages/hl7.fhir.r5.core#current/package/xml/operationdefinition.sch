...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:OperationDefinition">
      ...
      <sch:assert test="((f:kind/@value = 'query') and (f:/instance = false)) or (not(f:kind/@value = 'query'))">opd-5: A query operation cannot be defined at the instance level</sch:assert>
      <sch:assert test="((f:kind/@value = 'query') and ((f:parameter/f:use/@value = 'in') and (f:parameter/f:searchType))) or (not(f:kind/@value = 'query'))">opd-6: A query operation requires input parameters to have a search type</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="f:OperationDefinition//f:parameter">
      ...
      <sch:assert test="not(exists(f:searchType)) or ((f:use/@value = 'in')">opd-4: SearchParamType can only be specified on in parameters</sch:assert>
    </sch:rule>
  </sch:pattern>
</sch:schema>
