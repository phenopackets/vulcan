...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:CapabilityStatement">
      ...
      <sch:assert test="count(f:rest/f:mode/@value)=count(distinct-values(f:rest/f:mode/@value))">cpb-4: There should only be one CapabilityStatement.rest per mode.</sch:assert>
      ...
      <sch:assert test="not(exists(f:messaging/f:endpoint)) or f:kind/@value = 'instance'">cpb-3: Messaging end-point is only permitted when a capability statement is for an implementation.</sch:assert>
