...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:StructureDefinition">
      ...
      <sch:assert test="not(exists(f:baseDefinition)) or exists(f:derivation)">sdf-27: If there's a base definition, there must be a derivation </sch:assert>
    ...
    <sch:rule context="f:StructureDefinition/f:snapshot">
      ...
      <sch:assert test="count(f:element) = count(f:element[exists(f:definition) and exists(f:min) and exists(f:max)])">sdf-3: Each element definition in a snapshot must have a formal definition and cardinalities, unless model is a logical model</sch:assert>
    ...
    <sch:rule context="f:StructureDefinition/f:snapshot/f:element">
      <sch:assert test="empty(f:slicing) or f:slicing[f:discriminator | f:description]">sdf-28: If there are no discriminators, there must be a definition</sch:assert>
      ...
      <sch:assert test="matches(f:path/@value, '^[^\s\.,:;\'&amp;quot;\/|?!@#$%&amp;*()\[\]{}]{1,64}(\.[^\s\.,:;\'&amp;quot;\/|?!@#$%&amp;*()\[\]{}]{1,64}(\[x\])?(\:[^\s\.]+)?)*$')">eld-19: Element names cannot include some special characters with a max of 64 characters,</sch:assert>
      ...
      <sch:assert test="not(exists(f:binding)) or (count(f:type/f:code) = 0) or contains(f:type/f:code/@value, ':') or  f:type/f:code/@value=('code','Coding','CodeableConcept','Quantity','string', 'uri', 'Duration')">eld-11: Binding can only be present for coded elements, string, and uri if using FHIR-defined types</sch:assert>
    ...
    <sch:rule context="f:StructureDefinition/f:snapshot/f:element/f:type">
      <sch:assert test="not(exists(f:aggregation)) or exists(f:code[@value = 'Reference']) or exists(f:code[@value = 'canonical']) or exists(f:code[@value = 'CodeableReference'])">eld-4: Aggregation may only be specified if one of the allowed types for the element is a reference</sch:assert>
    ...
    <sch:rule context="f:StructureDefinition/f:snapshot/f:element/f:constraint">
      <sch:assert test="(f:/severity/@value != 'errror') or not(exists(f:suppress))">eld-26: Errors cannot be suppressed</sch:assert>
    </sch:rule>
    <sch:rule context="f:StructureDefinition/f:snapshot/f:element/f:binding">
      ...
      <sch:assert test="exists(f:description) or exists(f:valueSet)">eld-23: binding SHALL have either description or valueSet</sch:assert>
    </sch:rule>
