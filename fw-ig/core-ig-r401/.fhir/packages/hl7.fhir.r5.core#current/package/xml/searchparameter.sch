...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:SearchParameter">
      ...
      <sch:assert test="not(exists(f:xpath)) or exists(f:processingMode)">spd-1: If an expression is present, there SHALL be a processingMode</sch:assert>
      ...
      <sch:assert test="not(exists(f:comparator)) or (f:type/@value = 'number') or (f:type/@value = 'date') or (f:type/@value = 'quantity') or (f:type/@value = 'special')">spd-3: Search parameters comparator can only be used on type 'number', 'date', 'quantity' or 'special'.</sch:assert>
    </sch:rule>
